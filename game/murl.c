...
int murl_text_width(mu_Font font, const char *str, int len) {
  ...
  Vector2 size = MeasureTextEx(
      rlfont, str, rlfont.baseSize / GetWindowScaleDPI().x, MURL_TEXT_SPACING);
  return size.x;
}

int murl_text_height(mu_Font font) {
  Font rlfont = MURL_FONT_FROM_MU(font);
  return rlfont.baseSize / GetWindowScaleDPI().x;
}
...
void murl_render(mu_Context *ctx) {
  ...
  while (mu_next_command(ctx, &cmd)) {
    ...
    case MU_COMMAND_ICON: {
      ...
      Font rlFont = MURL_FONT_FROM_MU(ctx->style->font);
      // center text in rect
      Rectangle rect = MURL_RECTANGLE_FROM_MU(cmd->icon.rect);
      Vector2 text_size =
          MeasureTextEx(rlFont, icon, rlFont.baseSize / GetWindowScaleDPI().x,
                        ctx->style->spacing);
      Vector2 text_position = {
          rect.x + (rect.width - text_size.x) / 2,
          rect.y + (rect.height - text_size.y) / 2,
      };
      DrawTextEx(rlFont, icon, text_position,
                 rlFont.baseSize / GetWindowScaleDPI().x, ctx->style->spacing,
                 icon_color);
