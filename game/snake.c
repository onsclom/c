...
static short foodPos[2], snakePos[2],
    snakeDirection = 0, snakePartTime[ROWS][COLS] = {0}, snakeLength = 2,
    score = 0, gameOver = 0;
int main() {
  ...
  while (!WindowShouldClose()) {
    if (gameOver == 0) {
      snakePos[0] +=
          (snakeDirection == 0) ? 1 : ((snakeDirection == 1) ? -1 : 0);
      snakePos[1] +=
          (snakeDirection == 2) ? 1 : ((snakeDirection == 3) ? -1 : 0);
      ...
      snakeDirection =
          IsKeyDown(KEY_LEFT)
              ...
              : (IsKeyDown(KEY_RIGHT)
                     ...
                     : (IsKeyDown(KEY_UP)
                            ? 3
                            : (IsKeyDown(KEY_DOWN) ? 2 : snakeDirection)));
    ...
    if (gameOver == 0) {
      ...
      DrawText("You lost!",
               ...
               GetScreenHeight() / 2 - (MeasureText("You lost!", 50) / 2), 50,
               RED);
    EndDrawing();
  }
  CloseWindow();
